...
public class MainApp implements EntryPoint, HistoryListener {
	...
	public void onModuleLoad() {
		...
		if(Cookies.getCookie(AwConstants.cookieAuthToken) != null) {
			
			// If there is an auth token cookie, it needs to be validated

			awDataService.whoAmI(new AsyncCallback<String>() {

				/**
				 * If the server doesn't know about the user, intialize the login
				 * view.
				 */
				@Override
...
//					bind();
			...
			Cookies.removeCookie(AwConstants.cookieUserName);
			String it = extractView(initialToken);
			// NOTE #1: These tokens need to be handled here because they are separate from the main dashboard tokens
			// NOTE #2: History will fire the initial token, so we call History.fireCurrentHistoryState() manually
			if (HistoryTokens.register().equals(extractView(initialToken))) {
				History.fireCurrentHistoryState();
	...
	private void loadAppConfigAndInitApp() {
		this.awDataService.fetchAppConfig(new AsyncCallback<AppConfig>() {
			...
			public void onFailure(Throwable caught) {
				...
				ErrorDialog.show("Could not obtain server app config data", "Problem: the server may be unavailable or undergoing maintenance, please try again at a later time.");
			}
	...
	private void initApp(UserInfo userInfo, List<CampaignShortInfo> campaigns) {
		...
		History.newItem("explore_data", false); // changed to jump right to explore tab
		History.fireCurrentHistoryState();
	}
	...
	private void initLoginSelfRegistration() {
		this.awDataService.fetchAppConfig(new AsyncCallback<AppConfig>() {
			@Override
			public void onFailure(Throwable caught) {
				ErrorDialog.show("Could not obtain server app config data", "Uh oh. The server may be down or undergoing maintenance. Please try again at a later time.");
			}
	...
	private void initLoginRecovery() {
		this.awDataService.fetchAppConfig(new AsyncCallback<AppConfig>() {
			@Override
			public void onFailure(Throwable caught) {
				ErrorDialog.show("Could not obtain server app config data", "Phooey! The server may be down or undergoing maintenance. Please try again at a later time.");
			}
	...
	private void initLogin(final String loginMessage) {
		this.awDataService.fetchAppConfig(new AsyncCallback<AppConfig>() {
			@Override
			public void onFailure(Throwable caught) {
				ErrorDialog.show("Could not obtain server app config data", "Oops, The server may be down or undergoing maintenance. Please try again at a later time.");
			}
			...
			public void onSuccess(AppConfig appConfig) {
				...
				else if (appName.equalsIgnoreCase("routinesense"))
				{
					loginView = new LoginViewOhmageImpl();
				}
